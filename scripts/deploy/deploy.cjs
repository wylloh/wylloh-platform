...
async function deployContracts(network, signer) {
  ...
  try {
    ...
    const WyllohToken = await ethers.getContractFactory('WyllohToken', signer);
    const token = await WyllohToken.deploy("https://wylloh.com/api/metadata/");
    ...
    const WyllohMarketplace = await ethers.getContractFactory('WyllohMarketplace', signer);
    const marketplace = await WyllohMarketplace.deploy(token.address, signer.address);
