...
export class EthereumAdapter extends BaseChainAdapter {
  public async processTransferEvent(event: any): Promise<TokenEvent> {
    try {
      return {
        ...
        to: event.args.to,
        amount: 1, // ERC-721 transfers are always amount 1
      ...
      logger.error('Error processing Ethereum transfer event:', error);
      throw error;
    }
  }
  ...
  public async processListingEvent(event: any): Promise<ListingEvent> {
    try {
      ...
      logger.error('Error processing Ethereum listing event:', error);
      throw error;
    }
  }
  ...
  public async processPurchaseEvent(event: any): Promise<PurchaseEvent> {
    try {
      ...
      logger.error('Error processing Ethereum purchase event:', error);
      throw error;
    }
  }
  ...
  public async getTokenMetadata(tokenAddress: string, tokenId: string): Promise<Record<string, any>> {
    try {
      ...
      logger.error('Error getting Ethereum token metadata:', error);
      throw error;
    }
  }
} 